...
def train(args: Dict[str, str]):
    ...
    while True:
        ...
        for src_sents, tgt_sents in batch_iter(train_data, batch_size=train_batch_size, shuffle=True):
            ...
            # (batch_size,)
            loss = -model(src_sents, src_lens, tgt_sents, tgt_lens, teacher_forcing=tf_rate)
            loss.backward()
...
def apply_pretrained_embeddings(model, src_embedding_path, tgt_embedding_path):
    ...
    try:
        ...
        model.tgt_embedding.weight.requires_grad = False
        for word in tqdm(list(vocab.tgt.word2id.keys())):
            if word not in tgt_embedding_dict:
                continue
            word_idx = vocab.tgt.word2id[word]
            model.tgt_embedding.weight[word_idx, :] = torch.from_numpy(tgt_embedding_dict[word]).float()
            tgt_loaded_counts += 1
