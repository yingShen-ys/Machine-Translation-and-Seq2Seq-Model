...
class BasicLSTMSeq2seq(nn.Module):
    def __init__(self, embedding_size, hidden_size, vocab, bidirectional=True, dropout_rate=0.3):
        ...
        self.encoder = nn.LSTM(embedding_size, hidden_size, bidirectional=bidirectional, batch_first=True, dropout_rate=dropout_rate)
        self.decoder = nn.LSTM(embedding_size, hidden_size *2 if bidirectional else hidden_size, batch_first=True, dropout_rate=dropout_rate)
        self.hidden_layer = nn.Linear(hidden_size * 4 if bidirectional else hidden_size * 2, hidden_size * 2 if bidirectional else hidden_size * 1)
        self.output_layer = nn.Linear(hidden_size * 2 if bidirectional else hidden_size * 1, self.tgt_vocab_size)
    ...
    def decoder_lstm_cell(self, x, hc=None):
        '''
        A wrapper around the lstm to make its API the same as LSTM cell
        '''
        if hc is None:
            _, (h, c) = self.decoder(x)
        else:
            _, (h, c) = self.decoder(x, hc)
        return h, c
    ...
    def decode(self, src_states, final_states, src_lens, trg_tokens, trg_lens, teacher_forcing=0.5):
        ...
        '''
        tgt_input_vectors = self.tgt_embedding(trg_tokens[:, 1:, :])
        tgt_states = self.decoder(tgt_input_vectors, final_states) # (batch_size, tgt_max_len-1, hidden_size)

        attn_scores = torch.einsum('bij,bkj->bik', (tgt_states, src_states)) # (batch_size, tgt_max_len-1, src_max_len)
        idx = torch.arange(0, src_states.size(1), out=src_states.new(1).long()).unsqueeze(0)
        mask = (idx < src_lens.unsqueeze(1)-1).float() # make use of the automatic expansion in comparison, (batch_size, src_max_len)
        mask[:, 0] = 0 # exclude the <s> token
        # masked_attn_scores = attn_scores * mask.unsqueeze(1).expand_as(attn_scores) # (batch_size, tgt_max_len-1, src_max_len)

        # manual softmax with masking
        offset, _ = torch.max(attn_scores, dim=1, keepdim=True) # (batch_size, 1, src_max_len)
        exp_scores = torch.exp(attn_scores - offset) # numerical stability (batch_size, tgt_max_len-1, src_max_len)
        mask = mask.unsqueeze(1).expand_as(exp_scores)
        exp_scores = exp_scores * mask # (batch_size, tgt_max_len-1, src_max_len)
        attn_weights = exp_scores / torch.sum(exp_scores, dim=1, keepdim=True) # (batch_size, tgt_max_len-1, src_max_len)

        context_vectors = torch.einsum('bij,bjk->bik', (attn_weights, src_states)) # (b, t-1, s) X (b, s, h) -> (b, t-1, h)
        concatenated = self.dropout(torch.cat((tgt_states, context_vectors), dim=-1)) # (b, t-1, h*2)
        output_logits = self.output_layer(concatenated) # (b, t-1, vocab)
        tgt_gold_tokens = trg_tokens[:, 1:] # (b, t-1)
        nll = F.cross_entropy(output_logits, tgt_gold_tokens, reduction='none')

        # create masks, assuming padding is AT THE END
        idx = torch.arange(0, trg_tokens.size(1), out=trg_tokens.new(1).long()).unsqueeze(0)
        mask = (idx < trg_lens.unsqueeze(1)-1).float() # make use of the automatic expansion in comparison, exclude </s> token
    ...
    def greedy_search(self, src_sent, src_lens, beam_size=5, max_decoding_time_step=70, cuda=True):
        ...
        context_vector = BasicLSTMSeq2seq.compute_attention(h, src_states, src_lens,
        ...
        while decoding_step <= max_decoding_time_step and prd_token.item() != END_TOKEN_IDX:
            ...
            h, c = self.decoder_lstm_cell(vector, (h, c))
            context_vector = self.dropout(BasicLSTMSeq2seq.compute_attention(h, src_states, src_lens, attn_func=self.attn_func))
    ...
    def beam_search(self, src_sent, src_lens, beam_size=5, max_decoding_time_step=70, cuda=True):
        ...
        vector = self.trg_embedding(start_token) # (batch_size, embedding_size)
        h, c = self.decoder_lstm_cell(vector, (h, c))
        context_vector = BasicLSTMSeq2seq.compute_attention(h, src_states, src_lens, attn_func=self.attn_func) # (batch_size, hidden_size (*2))
        curr_logits = self.decoder_output_layer(torch.cat((h, context_vector), dim=-1)) # (batch_size, vocab_size)
        curr_ll = F.log_softmax(curr_logits, dim=-1) # transform logits into log-likelihoods
        ...
        for t in range(1, max_decoding_time_step):
            ...
            context_vector = BasicLSTMSeq2seq.compute_attention(h, src_states_tmp, src_lens, attn_func=self.attn_func)
    ...
    def compute_attention(curr_state, src_states, src_lens, attn_func, value_func=None):
        ...
        context_vector = torch.einsum('bij,bik->bjk', (value_vectors, attn_weights)).view(batch_size, -1)
...
class SimpleLSTMSeq2seq(nn.Module):
    ...
    def __init__(self, embedding_size, hidden_size, vocab, bidirectional=True, dropout_rate=0.3, label_smooth=1.0, num_layers=2):
        ...
        self.encoder_lstm = nn.LSTM(embedding_size, hidden_size, dropout=dropout_rate, bidirectional=bidirectional, num_layers=2, batch_first=True)
    ...
    def decode(self, src_states, final_states, src_lens, trg_tokens, trg_lens, teacher_forcing=0.5):
        ...
        context_vector = self.dropout(SimpleLSTMSeq2seq.compute_attention(h, src_states, src_lens, attn_func=self.attn_func)) # (batch_size, hidden_size (*2))
        curr_attn_vector = self.dropout(self.relu(self.decoder_hidden_layer(torch.cat((h, context_vector), dim=-1)))) # the thing to feed in input feeding
        ...
        for t in range(trg_tokens.size(-1)-2):
            ...
            h, c = self.decoder_lstm_cell(vector, (h, c))
            context_vector = self.dropout(SimpleLSTMSeq2seq.compute_attention(h, src_states, src_lens, attn_func=self.attn_func))
            curr_attn_vector = self.dropout(self.relu(self.decoder_hidden_layer(torch.cat((h, context_vector), dim=-1)))) # the thing to feed in input feeding
            curr_logits = self.decoder_output_layer(curr_attn_vector) # (batch_size, vocab_size)
    ...
    def greedy_search(self, src_sent, src_lens, beam_size=5, max_decoding_time_step=70, cuda=True):
        ...
        context_vector = self.dropout(SimpleLSTMSeq2seq.compute_attention(h, src_states, src_lens,
                                                       attn_func=self.attn_func))  # (batch_size, hidden_size (*2))
        curr_attn_vector = self.dropout(self.relu(self.decoder_hidden_layer(torch.cat((h, context_vector), dim=-1)))) # the thing to feed in input feeding
        curr_logits = self.decoder_output_layer(curr_attn_vector) # (batch_size, vocab_size)
        curr_ll = F.log_softmax(curr_logits, dim=-1)  # transform logits into log-likelihoods
    ...
    def beam_search(self, src_sent, src_lens, beam_size=5, max_decoding_time_step=70, cuda=True):
        ...
        for t in range(1, max_decoding_time_step):
            ...
            context_vector = self.dropout(SimpleLSTMSeq2seq.compute_attention(h, src_states_tmp, src_lens, attn_func=self.attn_func))
